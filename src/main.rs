...
async fn get_handler(State(state): State<Arc<AppState>>, uri: OriginalUri) -> Response {
    ...
    match path.as_str() {
        ...
        "/api/audit/events" => match state.service.store().audit_events(50).await {
        ...
        "/api/strategy/played" => match state.service.store().strategy_played_summary().await {
            Ok(summary) => Json(summary).into_response(),
            Err(error) => error_response(StatusCode::INTERNAL_SERVER_ERROR, error),
        },
    ...
    match path.as_str() {
        ...
        "/api/coupons/simulate/selected" => {
            ...
            match state
                .service
                .store()
                .paper_place_candidate_coupons(snapshot_id, stake, limit, max_open_exposure)
                .await
            {
                Ok(summary) => {
                    state
                        .service
                        .store()
                        .record_audit("paper_coupon_candidates_auto_placed", summary.clone())
                        .await
                        .ok();
                    Json(summary).into_response()
                }
