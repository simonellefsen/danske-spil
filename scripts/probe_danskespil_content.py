#!/usr/bin/env python3
"""Read-only Danske Spil Oddset content probe.

The script uses the same public content-service endpoint observed from the
anonymous Oddset page. It does not log in, click odds, submit coupons, or touch
account state.
"""
...
def date_boundaries(days: int) -> str:
    """Return upcoming UTC day boundaries accepted by the time-band endpoint."""
...
def market_kind(market: dict[str, Any]) -> str:
    ...
    if "special" in name or "special" in group or "specials" in name:
        return "special"
    if "hjørne" in name or "corner" in name or "corners" in group:
        return "corners"
    ...
    if "over" in name or "under" in name or "total" in group or "over_under" in group or " o " in f" {outcome_text} ":
        return "over_under"
    if "mål" in name or "goal" in group:
        return "goal"
    ...
    if "sæt" in name or "set" in name or "game" in name:
        return "set_or_game"
...
def normalize_event(event: dict[str, Any], max_markets: int) -> dict[str, Any]:
    ...
    for fact in commentary.get("facts") or []:
        participant = next(
            (
                item
                for item in commentary.get("participants") or []
                if item.get("id") == fact.get("participantId")
            ),
            {},
        )
        facts.append(
...
def is_relevant_event(event: dict[str, Any], config: dict[str, Any], include_live: bool) -> bool:
    ...
    haystack = " ".join(
        ...
        for value in (
            event.get("name"),
            (event.get("class") or {}).get("name"),
            (event.get("type") or {}).get("name"),
        )
    ).lower()
...
def summarize_sport(
    ...
    outrights = fetch_outright_events(config, limit=limit, max_markets=max_markets) if include_outrights else []
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Probe Danske Spil Oddset read-only content data.")
    parser.add_argument(
        "--sport",
        action="append",
        choices=sorted(SPORTS.keys()) + ["all"],
        default=None,
        help="Sport to probe. Repeatable. Defaults to all configured sports.",
    )
    parser.add_argument("--limit", type=int, default=6, help="Maximum events per sport.")
    parser.add_argument("--max-markets", type=int, default=8, help="Maximum markets per event.")
    parser.add_argument("--date-days", type=int, default=0, help="Optional upcoming UTC day boundaries to request.")
    parser.add_argument("--include-live", action="store_true", help="Keep started/live events in the result.")
    parser.add_argument("--no-outrights", action="store_true", help="Skip configured tournament/outright feeds.")
    parser.add_argument("--pretty", action="store_true", help="Pretty-print JSON.")
    args = parser.parse_args()

    requested = args.sport or ["all"]
    sports = sorted(SPORTS.keys()) if "all" in requested else requested
    result = {
        ...
        "sports": [
            summarize_sport(
                ...
                include_outrights=not args.no_outrights,
            )
            for sport in sports
        ],
    }
