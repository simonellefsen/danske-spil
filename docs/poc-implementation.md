...
- The API/web process proxies `/api/result-agent/queue` and `/api/result-agent/run` to `GAMBLER_RESULT_AGENT_URL` when configured, so UI-triggered result-agent work runs in the dedicated service rather than inside the web/API pod.
