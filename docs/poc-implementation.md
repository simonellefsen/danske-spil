...
- `scripts/sofascore_evidence_probe.py` is the local operator probe for browser-only Sofascore evidence. It opens the public match URL with `agent-browser`, extracts a compact final-score payload, and posts to the API only when not run with `--dry-run`. It defaults to `settle=false`.
