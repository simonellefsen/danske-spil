...
- `/api/performance` now includes `settlement_work.lookup_cadence`, which reports how many due paper positions and how much paper exposure have a recent lookup attempt, how many are due without a recent attempt, the last lookup time, and the next lookup due time.
- The same performance payload includes `settlement_work.lookup_due_items`, a capped operator queue of due paper singles or coupons whose latest lookup is missing or outside the cooldown window. Queue rows include stake so operators can prioritize stale result checks by exposure.
...
- The dedicated Rust result-agent deployment consumes `/api/result-agent/queue` on its own cadence and runs built-in Flashscore discovery for stale football, tennis, and basketball rows that do not yet have configured public result links. It stores the discovered source link and posts sanitized evidence through `POST /api/settlement/external-evidence` when a final score is available. Tennis doubles are matched by individual player ids on both sides; matched no-score terminal rows are treated as paper refunds with status-only audit evidence.
- Each result-agent queue task now also carries a `result_lookup_prompt` object for LLM/browser-assisted lookup. The prompt includes the event teams or players, sport, tournament/league context, competition, market, selected outcome, start time, expected result-check time, Europe/Copenhagen timezone hint, configured public source URLs, and a strict JSON response schema. It is paper-only and explicitly forbids login, private account data, real betting, or direct settlement submission.
- External result links and evidence store `tournament_name` when available, usually copied from the Danske Spil feed `competition` field. This keeps tennis tournaments, football leagues, basketball competitions, motorsports series, and golf events available to lookup workers without overloading participant names.
- The API/web process proxies `/api/result-agent/queue` and `/api/result-agent/run` to `GAMBLER_RESULT_AGENT_URL` when configured, so UI-triggered result-agent work runs in the dedicated service rather than inside the web/API pod. Run responses include queued, selected, attempted, and skipped paper exposure so capped cycles can be audited against queue priority.
- `GET /api/result-agent/queue` includes result-agent cycle health, the latest compact `result_agent_cycle_completed` audit event, and recent compact cycle summaries. The web UI renders the health note plus result-agent cycle tables beside the backlog.
