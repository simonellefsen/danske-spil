...
- `scripts/result_agent.py` consumes `/api/result-agent/queue` and runs the browser evidence probe for configured public result links, so stale rows do not require operator URL prompts.
