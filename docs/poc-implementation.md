...
- If a paper position is still not resulted or settled more than 24 hours after `expected_result_check_after`, the review loop escalates it to `external_result_required` and recommends `official_competition_results` as the next evidence source instead of continuing to rely only on stale Danske Spil content-feed state.
