...
- `POST /api/hermes/run` executes one safe Hermes loop cycle. The cycle refreshes the same paper-only daily reflection, refreshes replay evidence for open one-variable experiments without changing experiment status, summarizes active baseline/proposal counts, records a `hermes_cycle_completed` audit event, and returns the sanitized strategy and ledger context used.
- `POST /api/hermes/run` returns `replay_refresh` with refreshed/skipped experiment replay results. This step does not place paper bets, does not change experiment lifecycle status, and does not promote any baseline by itself.
- `GET /api/hermes` includes the latest `hermes_cycle_completed` audit event as `latest_cycle`, and the web UI renders it as a compact Hermes cycle summary for operator review.
...
- `GET /api/hermes` and `POST /api/hermes/run` include promotion gates for active experiments. These gates explain why promotion is blocked or eligible based on replay evidence, operator-reviewed status, settled paper sample size, aggregate unresolved exposure, latest reflection provisional/complete state, and paper-only safety.
- The web UI renders recent reflections as a compact table with the assessment fields that matter for operation: provisional/complete state, settlement progress, unresolved exposure, realized P/L, worst/best-case P/L, and promotion recommendation.
