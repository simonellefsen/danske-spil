...
- Result-agent cycle rows should show compact skipped/failure reasons, including Flashscore no-match diagnostic reasons and recommended next actions, so stale settlement failures can be fixed by improving aliases or source adapters without opening raw audit JSON.
